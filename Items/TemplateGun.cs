...
namespace Revolutions.Items
{
    ...
    public class TemplateGun : ModItem
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            // 这里可以写中文了ヾ(@^▽^@)ノ
            DisplayName.SetDefault("测试核心特效");

            // 物品的描述，加入换行符 '\n' 可以多行显示哦
            Tooltip.SetDefault("这还不错吧？");
        }
        ...
        {
            Dust dust = Dust.NewDustDirect(player.position, 1, 1, mod.DustType("LightFadeOut"), 0, 0, 0, Helper.GetRainbowColorLinear(a, 100), 0.38f);
            //Color color = Helper.GetRainbowColorLinear(a, 100);
            a++;
            if (a == 101) a = 1;
            position = Helper.RelativePositionTrans(player.position, new Vector2(190, 0).RotatedByRandom(MathHelper.ToRadians(360)));
            speedX = 0;
            speedY = 0;
            //Helper.Print(player.channel.ToString());
            Main.NewText("1", Color.White);
            return true;
        }
        ...
        {
            target.defDefense = -100000;
            target.dontTakeDamage = false;
            target.alpha = 0;
            target.velocity = new Vector2(0, 0);

        }
        /*public override bool ModifyHitNPC(Player player, NPC target, int damage, float knockBack, bool crit)
        {
            target.defDefense = -100000;
            target.dontTakeDamage = false;
            target.alpha = 0;
