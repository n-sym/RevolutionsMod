                ...
                if (current != Vector2.Zero)
                {
                    ...
                    for (int i = 0; i < Vector2.Distance(current, target) / 50; i++)
                    {
                        ...
                        for (int j = 0; j < 30; j++)
                        {
                            ...
                            spriteBatch.Draw(Main.projectileTexture[mod.ProjectileType("MeteowerHelper")], Helper.GetCloser(currentPos, targetPos, j, 30) - Main.screenPosition, null, Color.Multiply(color, 1), projectile.rotation, drawOrigin, 0.19f, SpriteEffects.None, 0f);
