    ...
    public class ThunderMagic : PowerProj
    {
        ...
        public override void AI()
        {
            ...
            foreach(NPC target in Main.npc)
            {
                ...
                {
                    hitednpc.Add(target);
                    //distance = Vector2.Distance(target.position, projectile.position);
                    Helper.Print(counter.ToString());
            ...
            {
                projectile.position = Helper.GetCloser(PositionSave[status], PositionSave[status + 1], fix, 3.5f);
                status++;
                fix++;
                if (fix == 5) fix = 0;
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            foreach (Vector2 current in PositionSave)
            {
                if (current != Vector2.Zero)
                {
                    ...
                    for (int i = 0; i < Vector2.Distance(current, target) / 4; i++)
                    {
                        ...
                        sizeFix /= hitednpc.Count * 250;
                        sizeFix *= projectile.timeLeft * 0.04f;
                        color *= sizeFix + 0.4f;
                        spriteBatch.Draw(Main.projectileTexture[mod.ProjectileType("MeteowerHelper")], Helper.GetCloser(current, target, i, Vector2.Distance(current, target) / 4) - Main.screenPosition, null, color, projectile.rotation, drawOrigin, 0.19f, SpriteEffects.None, 0f);
                    }
                }
            }
                /*for (int j = 0; j < 30; j++)
