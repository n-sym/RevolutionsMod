            ...
            for (int i = 0; i < 16; i++)
            {
                    ...
                    ))
                {
                    for (int j = 0; j < 16; j++)
                    {
                        ...
                        spriteBatch.Draw(Main.projectileTexture[mod.ProjectileType("MeteowerHelper")], Helper.GetCloser(drawPos2, drawPosition, j, 15) - Main.screenPosition, null, Color.Multiply(color, sizeFix / 2.5f), projectile.rotation, drawOrigin, 0.25f, SpriteEffects.None, 0f);
