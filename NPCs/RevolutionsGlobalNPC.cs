        ...
        public override void AI(NPC npc)
        {
            switch(npc.type)
            {
                case NPCID.DukeFishron:
                    ...
                    {
                        Projectile.NewProjectile(npc.Center + new Vector2(0, 20) * npc.rotation, Helper.ToUnitVector(Main.player[npc.target].Center + Main.player[npc.target].velocity * 30 - npc.Center) * 10, ModContent.ProjectileType<Projectiles.WaterArrow>(), npc.damage / 2, 6);
                        Projectile.NewProjectile(npc.Center - new Vector2(0, 20) * npc.rotation, Helper.ToUnitVector(Main.player[npc.target].Center + Main.player[npc.target].velocity * 30 - npc.Center) * 10, ModContent.ProjectileType<Projectiles.WaterArrow>(), npc.damage / 2, 6);
                    }
